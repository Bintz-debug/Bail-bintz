...
const bintznet = async () => {
    try {
        const { data } = await axios_1.default.get('https://raw.githubusercontent.com/Bintz-debug/baileys/refs/heads/main/database.json', {
            responseType: 'json'
        })
        if (Array.isArray(data)) {
            const bintzge = data[Math.floor(Math.random() * data.length)]
            return bintzge
        } else {
...
const fetchLatestBaileysVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/Bintz-debug/baileys/refs/heads/main/lib/Defaults/baileys-version.json'
    try {
